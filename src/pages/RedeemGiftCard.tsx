    ...
    try {
      ...
      if (error || !data.valid) {
        toast({
          ...
          description: data?.error || "Gift card not found or expired",
          variant: "destructive",
        });
      ...
      toast({
        ...
        description: `Balance: ₹${data.gift_card.balance.toLocaleString()}`,
      });
    } catch (err: any) {
      toast({
  ...
  const handleRedeem = async () => {
    if (!validatedCard || !selectedCampaign || !redeemAmount) {
      toast({
        ...
        description: "Please select a campaign and enter amount",
        variant: "destructive",
      });
      return;
    }
    ...
    if (amount > validatedCard.balance) {
      toast({
        ...
        description: `Maximum available: ₹${validatedCard.balance}`,
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `You planted ${data.redemption.trees_planted} trees!`,
      });
    } catch (err: any) {
      toast({
  ...
  if (redemptionResult) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-primary/5 to-background">
        <SEO
          title="Gift Card Redeemed - Himsols"
          description="Your gift card has been successfully redeemed for tree plantation."
        />
        <Navbar />
          ...
          <div className="max-w-lg mx-auto">
            <Card className="border-primary/20 shadow-xl">
              <CardHeader className="text-center bg-gradient-to-r from-primary/10 to-green-500/10 rounded-t-lg">
                ...
                <CardTitle className="text-2xl text-primary">Trees Planted! 🌳</CardTitle>
                <CardDescription>
                  Your gift made a real impact
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6 pt-6">
                ...
                <div className="bg-gradient-to-br from-primary to-green-600 text-white rounded-xl p-6 text-center">
                  <div className="text-5xl font-bold mb-2">
                    {redemptionResult.trees_planted}
                  </div>
                  <p className="text-lg opacity-90">Trees Planted</p>
                  <p className="text-sm opacity-75 mt-2">
                    for {redemptionResult.campaign_title}
                  </p>
                </div>
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between py-2 border-b">
                    <span className="text-muted-foreground">Amount Redeemed</span>
      ...
      <main className="container mx-auto px-4 py-24">
        ...
        <div className="text-center max-w-2xl mx-auto mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-3">
            Redeem Green Gift Card 🌱
          </h1>
          <p className="text-muted-foreground">
            Enter your gift card code and choose a campaign to support
          </p>
        </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-primary">
                        ₹{validatedCard.balance.toLocaleString()}
                      </p>
                    </div>
                    <Button 
                      variant="ghost" 
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setRedeemAmount(String(Math.min(500, validatedCard.balance)))}
                    >
                      ₹500
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setRedeemAmount(String(Math.min(1000, validatedCard.balance)))}
                    >
                      ₹1000
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setRedeemAmount(String(validatedCard.balance))}
                    >
                      Full Balance
                    </Button>
                  </div>
                ...
                {redeemAmount && parseFloat(redeemAmount) > 0 && (
                  <div className="bg-muted/50 rounded-lg p-4 text-center">
                    <p className="text-sm text-muted-foreground">
                      This will plant approximately{" "}
                      <strong className="text-primary">
                        {Math.floor(parseFloat(redeemAmount) / 499)} trees
                      </strong>
                    </p>
                  </div>
                )}
